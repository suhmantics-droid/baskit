...
- [x] **E2-6** Insights (monthly budget meter, lists-vs-caps, value by category). _(2026-07-21: shipped as the **Budget view** (sidebar → 💷 Budget): My money card (income − outgoings, DEVICE-LOCAL localStorage only, always opens masked, "use what's left" → account monthly budget), 6-month allocation table (cool-offs land in the month they end, over-budget flagged), category donut per selected month. Verified live incl. masked-on-return + maths. Lists-vs-caps already covered by sidebar bars + segcards.)_
- [x] **E2-7** PWA manifest + service worker + **share target** route (pre-fills add). _(Competitor research 2026-07-20: one-click capture is the lead feature for Karma/Moonsift/GiftList — extension + share-sheet is table stakes; prototype now surfaces the bookmarklet as "Save from any store" in the hero.)_ _(2026-07-21: manifest share_target → /add, sw.js (no caching; push handlers ready), one-tap capture runs the extraction ladder and saves without a form — verified live: shared River Island link → named, priced £46, imaged, in-stock item. Browser extension still future.)_
...
- [x] **E3-3** Headless/stealth fallback for blocked retailers. _(2026-07-22: shipped as a **Firecrawl stealth** fallback, not self-hosted Playwright — residential proxies get past the datacentre-IP blocks that headless-on-Vercel could not. `lib/extract/firecrawl.ts` + `extractWithFallback`: plain fetch first, escalate only on blocked/slow/200-unreadable, then our OWN free parser reads the returned HTML (no LLM credits). Opt-in via `FIRECRAWL_API_KEY` (in Vercel + .env). Verified live on prod: Argos £28, Currys £99, John Lewis £199.95, Boots £17.95 all via stealth; Amazon/Nike stay plain (no credit). **Fetch ~6/10 → ~10/10.** Cost: ~5 credits/blocked-scrape, beta fits the free 1,000/mo tier (£0). Research: `docs/spike/E3-3-scraper-research.md`.)_
...
- [ ] **E4-2** `/api/cron/dispatch-moments`: rank, batch, quiet hours, frequency cap. _(Interim 2026-07-21: pushes fire at creation inside the daily 06:00 UTC sweep — inherently a decent hour; a separate dispatcher matters once volume/channels grow.)_
- [x] **E4-3** Delivery: Web Push (VAPID) + email (Resend); deep-links with affiliate + disclosure. _(2026-07-21: Web Push DONE — VAPID keys, subscribe API, avatar-menu opt-in, cron pings genuinely-new moments only, dead endpoints pruned (verified). Email digest DEFERRED until Resend domain verification; affiliate links not yet in deeplinks. NEEDS: VAPID_PUBLIC_KEY/VAPID_PRIVATE_KEY/VAPID_SUBJECT envs in Vercel.)_
- [x] **E4-4** Moments/Activity feed UI + per-moment snooze/mute + "why am I seeing this?". _(2026-07-21: bell + badge + feed with seen/clicked/dismissed; body text carries the why. Snooze deferred with E4-2.)_
